...
public class ModuleStructure {
	...
	public void registerFile(String id, String path, RegistrationCallback callback) throws IOException {
		...
		// If the file is already in the current structure, rejects it with the
		// current owner
		if (isRegistered(path)) {
	...
	private Dependency findDependency(Dependency dependency, List<Dependency> dependencies) {
		...
		while (it.hasNext()) {
			...
			if (dependency.getGroupId().equals(dep.getGroupId())
					...
					&& ((dependency.getClassifier() == null && dep.getClassifier() == null) || (dependency.getClassifier() != null && dependency
							.getClassifier().equals(dep.getClassifier())))) {
				return dep;
			}
		}
		return null;
	...
	public interface RegistrationCallback {
		 ...
		 * @param targetFilename
		 *            the relative path according to the root of the webapp
		 * @param deprecatedOwnerId
		 *            the previous owner that does not exist anymore
		 * @throws IOException
		 *             if an error occurred while handling this event
		 */
